...
def main():
    ...
    parser.add_argument(
        ...
        help='Base address where the software will be loaded, usually in RAM. '
             'Supply in hexadecimal format. Min 0x02000000, max 0x027FFFFF.'
    )
    parser.add_argument(
        '-e', '--exec',
    ...
    if not (0x02000000 <= base <= 0x027FFFFF):
        raise ValueError(
            'Base has invalid value. Minimum 0x02000000, maximum 0x027FFFFF.'
        )    
    ...
    if not (2 <= filesize <= 8388608):
        raise ValueError(
            'Size of file is invalid. Minimum 2 bytes, maximum 8388608.'
        )

    if base + filesize > 0x027FFFFF:
        raise ValueError(
